...
public class Serial {
    ...
    public Serial(String serialPort, Baud baudRate) {
        ...
        try {
            ...
            System.out.println("Serial " + serialPort + " init fail at baud " + baudRate.getValue() + " : " + e.getMessage());
        }
    }
    ...
    public Serial(String serialPort, int baudRate) {
        ...
        System.out.println("Serial " + serialPort + " init at baud " + baudRate);
        this.serialPort = serialPort;
        serial = SerialFactory.createInstance();
        try {
            serial.open(serialPort, baudRate);
        } catch (IOException e) {
            System.out.println("Serial " + serialPort + " init fail at baud " + baudRate + " : " + e.getMessage());
        }
    }
    ...
    public void write(String string) {
        try {
            System.out.println("Serial " + serialPort + " write : " + string);
            serial.writeln(string);
        } catch (IOException e) {
            System.out.println("Serial " + serialPort + " write fail : " + e.getMessage());
        }
    }

    /**
     * Envoie une string sur la liaison série
     * @param bytes String à envoyer
     */
    public void write(byte[] bytes) {
        try {
            System.out.println("Serial " + serialPort + " write "+bytes.length+" byte(s)");
            serial.write(bytes);
            serial.flush();
        } catch (IOException e) {
            System.out.println("Serial " + serialPort + " write fail : " + e.getMessage());
